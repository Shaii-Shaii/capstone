...
npx.cmd supabase secrets set OPENROUTER_WIG_HEAD_DETECTION_MODEL="google/gemini-3.1-flash-lite"
npx.cmd supabase secrets set OPENROUTER_IMAGE_MODEL="black-forest-labs/flux.2-pro"
...
- `black-forest-labs/flux.2-pro` is used through OpenRouter for the patient wig try-on image edit. Image generation is billed and requires OpenRouter credits.
...
- Wig-preview image generation requires OpenRouter and `black-forest-labs/flux.2-pro` in `OPENROUTER_IMAGE_MODEL`; it will not silently fall back to another image provider.
